...
export default function CreatePostForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    returnTimeline: "", // UC Supplement requirement
    lookingFor: "", // UC Step 5
    partialCash: false, // UC Step 5 toggle
    termsAgreed: false, // UC Supplementary requirement
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const postData = {
        ...formData,
        exchangeType:
          formData.postType === "SERVICE" ? "PERMANENT" : formData.exchangeType,
        returnTimeline:
          formData.postType === "SERVICE" ? "N/A" : formData.returnTimeline,
        serviceQualification:
          formData.postType === "ITEM" ? "N/A" : formData.serviceQualification,
      ...
      <section className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
        ...
        <div className="space-y-4">
          <input
            ...
            className="w-full text-2xl font-extrabold text-slate-900 outline-none placeholder:text-slate-200 focus:placeholder:text-slate-100"
            onChange={(e) =>
        ...
        {formData.postType === "ITEM" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-slate-50 animate-in fade-in duration-300">
            <div>
              ...
              <div className="flex p-1.5 bg-slate-100 rounded-2xl">
                {(["PERMANENT", "TEMPORARY"] as const).map((type) => (
                  <button
                    ...
                    className={`flex-1 py-3 text-xs font-black rounded-xl transition-all ${formData.exchangeType === type ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500"}`}
                  >
                    {type}
                  </button>
                ))}
            ...
            {/* Return Timeline (Only for Temporary Items) */}
            {formData.exchangeType === "TEMPORARY" && (
              <div className="animate-in slide-in-from-left-4 duration-300">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, returnTimeline: e.target.value })
                  }
                />
              </div>
            )}
